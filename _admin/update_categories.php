...
include('handling/handling_update_categories.php');
                        ...
                        <div class="col-lg-12">
                            <div class="card mb-4">
                                ...
                                <form class="forms-sample ml-5 mb-3" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
                                    <?php
                                    $query_all_category = mysqli_query($con, "SELECT product_type_id, product_type_name, coupons.coupon_id, coupon_name FROM product_types, coupons WHERE product_types.coupon_id=coupons.coupon_id AND product_type_id=" . $_SESSION['update_by_id'] . ";");
                                        ...
                                        <div class="row">
                                            <div class="col-8">
                                                <div class="form-group">
                                                    ...
                                                    <input name="product_type_id" type="text" class="form-control" id="" value="<?php echo htmlentities($row['product_type_id']); ?>" disabled >
                                                </div>
                                                <div class="form-group">
                                                    <label for="">Category name</label>
                                                    <input name="product_type_name" type="text" class="form-control" id="" value="<?php echo htmlentities($row['product_type_name']); ?>">
                                                </div>
                                                <div class="form-group">
                                                    <label for="">Coupon name</label>
                                                    <select class="form-control" name="coupon_id">
                                                        <?php
                                                        $query_all_coupons = mysqli_query($con, "SELECT coupon_id, coupon_name FROM coupons;");
                                                        while ($row_coupon = mysqli_fetch_array($query_all_coupons)) {
                                                            ?>
                                                            <?php if ($row_coupon['coupon_name'] == $row['coupon_name']) { ?>
                                                                <option value="<?php echo htmlentities($row_coupon['coupon_id']); ?>" selected><?php echo htmlentities($row_coupon['coupon_name']); ?></option>
                                                            <?php } else { ?>
                                                                <option value="<?php echo htmlentities($row_coupon['coupon_id']); ?>"><?php echo htmlentities($row_coupon['coupon_name']); ?></option>
                                                            <?php } ?>
                                                        <?php } ?>
                                                    </select>
                                                </div>   
                                    ...
                                    <a href="view_categories.php" class="btn btn-light">Cancel</a>
                                </form>
                            </div>
                        </div>
