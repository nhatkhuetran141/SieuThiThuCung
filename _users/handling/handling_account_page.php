...
if (isset($_POST['update'])) {
    ...
    $customer_bio = test_input($_POST['customer_bio']);
        ...
        if (preg_match('/^[0-9]+$/', $phone) || $phone == '') {
            if (preg_match('/([0-9]{4})\-([0-9]{2})\-([0-9]{2})/', $date_of_birth) || $date_of_birth == '') {
                ...
                $update_account = mysqli_query($con, "UPDATE accounts SET account_name='$account_name', date_of_birth='$date_of_birth', account_address='$account_address', avatar='$file_old', "
                        . "phone='$phone', gender='$gender', update_date_account='$currentTime' WHERE account_id='" . $_SESSION['account_id'] . "';");
                $update_customer = mysqli_query($con, "UPDATE customers SET customer_bio='$customer_bio' WHERE account_id='" . $_SESSION['account_id'] . "';");

                if ($update_account && $update_customer) {
